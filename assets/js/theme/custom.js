...
export default function customJS(context) { 
    ...
    //most_popular Slider Dots click show product(LookBook Section) start
            ...
            if(graphql_token != "" && graphql_token != "undefined" && finalproductIds != "") { 
                ...
                .then((res) => {
                    if(res.data != null){
                            ...
                            mp_slider_1_data_spt.length > 0 && mp_slider_1_data_spt.map((ids)=>{
                                productsGq.length > 0 && productsGq.map((list) => {
                                    ...
                                    if(ids == entityId){
                                        ...
                                        let priceHtml = "";
                                        //JRivas 19/03/2023
                                        const basePriceWithTax = (basePrice * 1.18).toFixed(2);
                                        const salePriceWithTax = salePrice ? (salePrice * 1.18).toFixed(2) : null;

                                        if(typeof salePrice != "undefined" && salePrice != null){
                                            priceHtml += `<div class="price-section d-inline-block price-section--withTax"><span data-product-price-with-tax="" class="price price--withTax">${symbol}${salePriceWithTax}</span></div>`;
                                            priceHtml += `<div class="price-section d-inline-block price-section--withTax rrp-price--withTax"><span data-product-rrp-with-tax="" class="price price--rrp">${symbol}${basePriceWithTax}</span></div>`;
                                        }else{
                                            priceHtml += `<div class="price-section d-inline-block price-section--withTax"><span data-product-price-with-tax="" class="price price--withTax">${symbol}${basePriceWithTax}</span></div>`;
                                        }

                                        console.log('Valor de priceHtml', priceHtml);
            
                                        let addToCartUrl_html = "";
                                        if(productOptions.length > 0){
                                            addToCartUrl_html += `<a href="${path}">Comprar</a>`;
                                        }else{
                                            addToCartUrl_html += `<a href="${addToCartUrl}">Agregar al Carrito</a>`;
                                        }
                                        child_html_1 += `<div class="mp-card" data-id="${entityId}">
                                            <div class="mp-card-inner">
                                                <a href="${path}" class="mp-card-innner-link">
                                                    <img class="mp-card-image" src="${defaultImage}" alt="${name}" width="auto" height="auto" />
                                                </a>
                                ...
                                productsGq.length > 0 && productsGq.map((list) => {
                                    ...
                                    if(ids == entityId){
                                        ...
                                        child_html_2 += `<div class="mp-card" data-id="${entityId}">
                                            <div class="mp-card-inner">
                                                <a href="${path}" class="mp-card-innner-link"><img class="mp-card-image" src="${defaultImage}" alt="${name}" width="auto" height="auto" /></a>
                        ...
                        if ($(window).width() >= 801) {  
                            $(".product_section .mp-child-slider").slick(({arrows: false,dots: true, slidesToShow: 2,slidesToScroll: 1}));   
                        }else{
    ...
    //most_popular Slider Dots click show product(LookBook Section) End
